...
{
    public partial class SearchByTel : Page
    {
        ...
        protected void Searchbtn_Click(object sender, EventArgs e)
        {
            ...
            if (!(searchtel.Text.Length >= 0 && searchtel.Text.Trim().Length == 0))//Ensures that the textbox is not null and is clear of spaces
            {
                ...
                finally//After all the DB is read
                {
                    ...
                    //Searches all the contacts, and picks those that their phone number matches with the user's input. The contacts picked are added to a new list
                    foreach (Contact contact in contactsList)
                        if (contact.Ctel == searchtel.Text)
