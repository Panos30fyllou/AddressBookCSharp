...
namespace AddressBook
{
    public partial class Contact
    {
        ...
        public Contact(string cfname, string clname, string ctel, string cemail, string caddress, string cbday)
        {
            this.cfname = cfname;
            this.clname = clname;
            this.ctel = ctel;
        ...
        public string Cbday { get => cbday; set => cbday = value; }
        public string Cfname { get => cfname; set => cfname = value; }
    ...
    public partial class SearchByFName : Page
    {
        ...
        protected void Searchbtn_Click(object sender, EventArgs e)
        {
            ...
            if (!(searchfname.Text.Length >= 0 && searchfname.Text.Trim().Length == 0))//Ensures that the textbox is not null and is clear of spaces
            {
                ...
                finally//After all the DB is read
                {
                    ...
                    //Searches all the contacts, and picks those that their first name matches with the user's input. The contacts picked are added to a new list
                    foreach(Contact contact in contactsList)                
                        if (contact.Cfname == searchfname.Text)                        
                        ...
                        if (valideimail) {
                            if (!(addresstxtbox.Text.Length >= 0 && addresstxtbox.Text.Trim().Length == 0)) {                                
                                if (DateTime.TryParse(bdaytxtbox.Text, out DateTime dt))
                                {
                                    ...
                                    {
                                        //After we ensure the textbox values are valid, and there is no contact with the same phone number as the new one, the old contact is deleted from the database
        ...
        ///NEXT CONTACT///
        //////////////////
        public void nextcontact_Click(object sender, EventArgs e)
        {
